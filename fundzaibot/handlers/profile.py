...
Regular users now see their ecosystem membership status (channel + group).
"""
...
async def profile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/profile — full user stats card including ecosystem membership status."""
    ...
    if is_admin(uid):
        ...
        text = (
            ...
            f"<b>Name:</b> {html.escape(user.first_name or 'N/A')}\n"
    ...
    lang   = db_user.get("language", "en").upper()

    # ── Ecosystem membership status ───────────────────────────────────────────
    membership_lines = []
    from config.settings import TELEGRAM_CHANNEL_ID, TELEGRAM_GROUP_ID

    if TELEGRAM_CHANNEL_ID or TELEGRAM_GROUP_ID:
        try:
            from handlers.membership import check_membership
            status = await check_membership(context.bot, uid, context.bot_data)
            if TELEGRAM_CHANNEL_ID:
                chan_name = TELEGRAM_CHANNEL_NAME or "Channel"
                icon = "✅" if status["channel"] else "❌"
                membership_lines.append(f"  📢 {html.escape(chan_name)}: {icon}")
            if TELEGRAM_GROUP_ID:
                grp_name = TELEGRAM_GROUP_NAME or "Community"
                icon = "✅" if status["group"] else "❌"
                membership_lines.append(f"  👥 {html.escape(grp_name)}: {icon}")
        except Exception:
            pass
    ...
    text = (
        f"👤 <b>Your Profile</b>\n\n"
        f"<b>Name:</b> {html.escape(user.first_name or 'N/A')} {html.escape(user.last_name or '')}\n"
...
async def history_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for i, img in enumerate(images, 1):
        lines.append(
            f"{i}. <b>{html.escape(img.get('style', '').capitalize())}</b> — "
            f"{html.escape(img.get('prompt', '')[:60])}…\n"
