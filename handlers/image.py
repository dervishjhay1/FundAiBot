...
async def image_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Admin bypasses VIP expiry check — always treated as unlimited
...
async def _run_generation(
    ...
    # Generate image in executor so the event loop is never blocked
    ...
    if image_buf:
        ...
        log.info("Image sent: user=%s admin=%s style=%s", user_id, admin, style)
    else:
        await loop.run_in_executor(
            None,
